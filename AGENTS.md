...
- `src/lib/` holds the shared singletons: `prisma.ts`, `supabase.ts` (browser client, server singleton, and the request-scoped `createSupabaseRequestClient()` factory), `ai.ts` (model), `fal.ts` (fal.ai client), `browserless.ts` (PDF-export client) — plus `api-auth.ts` (session → Prisma `User` helper), `env.ts` (call-time env validation), `utils.ts`, and ~60 pure logic modules, grouped thematically: schemas (`checklist-schema`, `room-patch-schema`, `room-copy-edit-schema`, `settings-schema`, `ai-route-schemas`, `metadata-schemas`, `sign-project-schema`, `staging-packages-schema`); inpaint pipeline (`inpaint-polling`, `inpaint-persistence`, `inpaint-source`, `inpaint-completion`, `inpaint-quality-gate`, `staged-result`, `no-object-error`, `error-classify`, `prompts`); mask ops (`mask-coverage`, `mask-flood-fill`, `mask-dilation`, `mask-format`, `mask-postprocess`, `holistic-mask`, `holistic-preset`, `holistic-prompt`); segmentation/vision (`segment-mask`, `segment-cache`, `segment-timing`, `furnishing-detection`, `vision-labels`, `instance-hit-test`); canvas/studio UI logic (`canvas-coords`, `tool-rail`, `precision-slider`, `workbench-layout`, `inspector-panel`, `split-comparison-canvas`, `multi-select-batch`, `autosave-controller`, `focus-mode`, `studio-workflow`, `variant-legibility`, `thumbnail-data-url`, `prompt-prefill`, `consultation-action-bar`); lookbook/branding content (`lookbook-pillars`, `staging-aesthetics`, `aesthetic-chips`, `concept-chips`, `moodboard-themes`, `engagement-tiers`, `color-tokens`, `typography-tokens`); auth/access (`auth-redirect`, `login-error`, `preview-token`, `preview-access`); dashboard/project data (`dashboard-data`, `project-fetch-state`, `project-filter`); and `api-quota` (mutation quota tracking). Pattern: extract pure logic into `src/lib/<name>.ts` and pin it with a 1:1 `tests/<name>.test.ts` — 57 of the current 69 modules are pinned that way. Exceptions: the infra files (`prisma`, `supabase`, `ai`, `fal`, `env`, `api-auth`, `utils`) have no unit test by convention; `ai-route-schemas` is pinned by two differently-named tests (`generate-copy-request-schema.test.ts`, `segment-request-schema.test.ts`); `concept-batch-toggle.test.ts` pins `multi-select-batch`; `preview-access.test.ts` also covers `preview-token`; `dashboard-data`, `staging-aesthetics`, and `staging-packages-schema` currently have no unit test at all. A few other name-mismatched tests pin components or API routes instead of lib modules (e.g. `comparison-slider`, `room-batch-card-matrix`, `inpaint-submit-route`, `inpaint-status-route`).
