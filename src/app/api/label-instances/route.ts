...
const log = componentLogger("api:label-instances");
...
export const POST = withErrorHandler(async (request: NextRequest) => {
  return withNextRouteLogging(request, null, async () => {
    ...
    if (!labelQuota.allowed) {
      log.warn(
        {
          type: "label_instances_daily_quota_exceeded",
          userId: user.id,
          used: labelQuota.used,
          limit: labelQuota.limit,
        },
        "Daily label instances quota exceeded"
      );
      throw new ApiError({
    ...
    try {
      ...
      trackAIError({
        err: error,
        model: "gpt-4o-mini",
        context: { route: "label-instances", concept, roomId },
      });
      throw new ApiError({
